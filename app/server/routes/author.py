...
from app.server.models.author import AuthorModel, UpdateAuthorModel
...
@router.get('/', response_description='List all authors of authors', response_model=List[AuthorModel])
async def get_author_list() -> JSONResponse:
    authors = await get_authors()
    return JSONResponse(status_code=status.HTTP_200_OK, content=authors)


@router.get('/{author_id}', response_description='Get an author with given id', response_model=AuthorModel)
async def get_one_author(author_id: str) -> JSONResponse:
...
@router.delete('/{author_id}', response_description='Delete an author from database')
